...
```{r fig.width = 10, fig.height=25, echo=FALSE, warning=FALSE, message=FALSE}
...
plot <- df_summ |>
  ...
  geom_text(aes(x = 0.1, label = toupper(aesthetic)), size = 5, hjust = 0, vjust = 0.5) +
  geom_text(aes(x = 0.05, label = rank), size = 3.5) +
  geom_text(aes(x = 0.65, label = paste0(scales::comma(max_date)," results")), hjust = 1) +
  geom_text(aes(x = 0.8, label = scales::percent(diff_perc, 0.1)), hjust = 0.5, size = 4) +
